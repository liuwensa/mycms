...
var siteFunc   = {
  siteInfos: function (title, cmsDescription, keyWords) {
    return {
      title         : `${title}|${config.siteInfo.SITETITLE}`,
      cmsDescription: cmsDescription? cmsDescription : siteInfo.CMSDISCRIPTION,
      keywords      : keyWords? `{keyWords},${siteInfo.SITEBASICKEYWORDS}` : siteInfo.SITEKEYWORDS,
      siteIcp       : siteInfo.SITEICP,
      version       : siteInfo.SITEVERSION
    }
  },

  setConfirmPassWordEmailTemp: function (name, token) {
    var html = '<p>您好：' + name + '</p>' +
      '<p>我们收到您在 <strong>' + siteInfo.SITETITLE + '</strong> 的注册信息，请点击下面的链接来激活帐户：</p>' +
      '<a href="' + siteInfo.SITEDOMAIN + '/users/reset_pass?key=' + token + '">重置密码链接</a>' +
      '<p>若您没有在 <strong>' + siteInfo.SITETITLE + '</strong> 填写过注册信息，说明有人滥用了您的电子邮箱，请忽略或删除此邮件，我们对给您造成的打扰感到抱歉。</p>' +
      '<p> <strong>' + siteInfo.SITETITLE + ' </strong> 谨上。</p>';
  ...
  setNoticeToAdminEmailTemp: function (obj) {
    ...
    return '主人您好，<strong>' + obj.author.userName + '</strong> 于 '
      + msgDate + ' 在 <strong>' + siteInfo.SITETITLE + '</strong> 的文章 <a href="'
      + siteInfo.SITEDOMAIN + '/details/' + obj.contentId + '.html">'
      + obj.contentTitle + '</a> 中留言了';
  },
  ...
  setNoticeToUserEmailTemp: function (obj) {
    ...
    html += '主人您好，<strong>' + targetEmail + '</strong> 于 ' + msgDate + ' 在 <strong>' + siteInfo.SITETITLE + '</strong> '
      + '的文章 '
      + '<a href="' + siteInfo.SITEDOMAIN + '/details/' + obj.contentId + '.html">' + obj.contentTitle + '</a> 中回复了您';
    return html;
  },
  ...
  setBugToAdminEmailTemp: function (obj) {
    ...
    var html    = '';
    html += '主人您好，测试管理员（' + obj.email + ')于 ' + msgDate + ' 在 <strong>' + siteInfo.SITETITLE + '</strong> 的后台模块 <strong>' + obj.contentFrom + '</strong> 中说：<br>' + obj.content;
    return html;
  },

  setNoticeToUserRegSuccess: function (obj) {
    var html = '';
    html += '亲爱的 ' + obj.userName + ' （' + obj.email + ') ，恭喜您成为 <strong>' + siteInfo.SITETITLE + '</strong> 的新用户！ 您现在可以 <a href="' + siteInfo.SITEDOMAIN + '/users/login" target="_blank">点击登录</a>';
    return html;
  },

  getCategoryList: function () {
    return db.ContentCategory.find({'parentID': '0', 'state': '1'}, 'name defaultUrl').sort({'sortId': 1}).find();
  },

  getHotItemListData: function (q) {
    return db.Content.find(q, 'stitle').sort({'clickNum': -1}).skip(0).limit(10);
  },

  getNewItemListData: function (q) {
    return db.Content.find(q, 'stitle').sort({'date': -1}).skip(0).limit(10);
  },

  getRecommendListData: function (cateQuery, contentCount) {
    return db.Content.find(cateQuery).sort({'date': -1}).skip(Math.floor(contentCount * Math.random())).limit(4);
  },

  getFriendLink: function () {
    return db.Ads.find({'type': '2'}).populate('items').exec();
  },

  getMessageList: function (contentId) {
    return db.Message.find({'contentId': contentId}).sort({'date': 1}).populate('author').populate('replyAuthor').populate('adminAuthor').exec();
  },
  ...
  sendSystemNoticeByType: function (req, res, type, value) {
    ...
    Notify.sendSystemNotice(res, noticeObj, function (users, notify) {
      db.UserNotify.addNotifyByUsers(res, users, notify);
    });
  },
  ...
  setDataForIndex: function (req, res, params, staticforder, defaultTempPath) {
    ...
    var documentList = db.DbOpt.getPaginationResult(db.Content, req, res, params, requireField);
    var tagsData     = db.DbOpt.getDatasByParam(db.ContentTags, req, res, {});
    return {
  ...
  setDataForCate: function (req, res, params, staticforder, defaultTempPath) {
    ...
    var documentList    = db.DbOpt.getPaginationResult(db.Content, req, res, params.contentQuery, requireField);
    var currentCateList = db.ContentCategory.find(params.cateQuery).sort({'sortId': 1});
    var tagsData        = db.DbOpt.getDatasByParam(db.ContentTags, req, res, {});
    return {
  ...
  setDataForSearch: function (req, res, params, staticforder, defaultTempPath) {
    ...
    var documentList    = db.DbOpt.getPaginationResult(db.Content, req, res, params.query, requireField);
    return {
  ...
  setDataForUserReply: function (req, res, params, staticforder, defaultTempPath) {
    req.query.limit  = 5;
    var documentList = db.DbOpt.getPaginationResult(db.Message, req, res, {'author': req.session.user._id});
    return {
      siteConfig  : this.siteInfos(params.title),
      cateTypes   : this.getCategoryList(),
      userInfo    : req.session.user,
  ...
  setDataForUserNotice: function (req, res, params, staticforder, defaultTempPath) {
    req.query.limit  = 10;
    var documentList = db.UserNotify.getNotifyPaginationResult(req, res, req.session.user._id);
    return {
      siteConfig        : this.siteInfos(params.title),
      cateTypes         : this.getCategoryList(),
      userInfo          : req.session.user,
  ...
  setDataForSiteMap: function (req, res) {
    ...
    db.ContentCategory.find({}, 'defaultUrl', function (err, cates) {
      if (err) {
        console.log(err);
      } else {
        ...
        db.Content.find({}, 'title', function (err, contentLists) {
          if (err) {
            console.log(err);
          } else {
