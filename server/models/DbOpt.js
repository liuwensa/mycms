...
var DbOpt = {
  del          : function (obj, req, res, logMsg) {
    ...
    if (shortid.isValid(targetId)) {
      ...
      res.end(settings.systemIllegalParam);
    }
  ...
  // 通用查询list不带分页，注意参数传递格式,通过express-promise去掉了回调方式返回数据
  ...
  getKeyArrByTokenId: function (tokenId) {
    var newLink = DbOpt.decrypt(tokenId, config.encryptKey);
