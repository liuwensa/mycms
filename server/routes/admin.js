...
router.get('/', adminUserCtr.adminLogin);

//刷新验证码
router.get('/vnum', adminUserCtr.verificationCode);

// 管理员登录提交请求
router.post('/doLogin', adminUserCtr.doLogin);
...
router.get('/manage/:defaultUrl/del', function (req, res) {
  ...
  } else if (targetObj == db.ContentTemplate) {
    removeTemplate(req, res);
  } else if (targetObj == db.Ads) {
    removeAds(req, res);
  } else if (targetObj == db.TemplateItems) {
...
router.get('/manage/:defaultUrl/batchDel', function (req, res) {
  ...
  if (targetObj == db.Message || targetObj == db.AdminGroup || targetObj == db.AdminUser || targetObj == db.Notify) {
...
router.get('/manage/:defaultUrl/item', function (req, res) {
  ...
  if (targetObj == db.AdminUser) {
    db.AdminUser.getOneItem(res, targetId, function (user) {
      return res.json(user);
    });
  } else if (targetObj == db.Ads) {
    db.Ads.getOneAds(res, targetId, function (doc) {
      return res.json(doc);
    });
  } else {
    db.DbOpt.findOne(targetObj, req, res, "find one obj success");
  }
});
...
router.post('/manage/:defaultUrl/modify', function (req, res) {
  ...
  if (targetObj == db.AdminUser || targetObj == db.User) {
    req.body.password = db.DbOpt.encrypt(req.body.password, config.encryptKey);
  } else if (targetObj == db.AdminGroup) {
    if (params.query.uid == req.session.adminUserInfo.group._id) {
      req.session.adminPower = req.body.power;
    }
  } else if (targetObj == db.ContentCategory) {
    db.ContentCategory.updateCategoryTemps(req, res, params.query.uid);
  }
  db.DbOpt.updateOneByID(targetObj, req, res, "update one obj success")
});
...
router.post('/manage/:defaultUrl/addOne', function (req, res) {
  ...
  } else if (targetObj == db.Content) {
    req.body.author = req.session.adminUserInfo._id;
    db.DbOpt.addOne(targetObj, req, res);
  } else if (targetObj == db.ContentTags) {
...
router.get('/manage/adminGroupList/list', function (req, res) {
  db.DbOpt.findAll(db.AdminGroup, req, res, "request adminGroupList")
});
...
router.get('/manage/contentCategorys/list', function (req, res) {
  return res.json(db.ContentCategory.find({}).sort({'sortId': 1}));
});

//所有标签列表
router.get('/manage/contentTags/list', function (req, res) {
  db.DbOpt.findAll(db.ContentTags, req, res, "request ContentTags List")
});

//所有默认模板列表
router.get('/manage/contentTemps/list', function (req, res) {
  db.ContentTemplate.getDefaultTemp(res, function (doc) {
    if (doc) {
...
function removeMessage(req, res) {
  ...
  db.Message.findOne({_id: targetId}, 'contentId', function (err, result) {
    if (err) {
      res.end(err);
    } else {
      if (result && result.contentId) {
        var contentId = result.contentId;
        db.Content.findOne({_id: contentId}, function (err, contentObj) {
          if (err) {
            res.end(err);
          } else {
            db.Message.remove({_id: params.query.uid}, function (err) {
...
function addOneAdminUser(req, res) {
  ...
  if (utils.validator.isUserName(userName)) {
    db.AdminUser.findOne({userName: req.body.userName}, function (err, user) {
      if (user) {
        errors = "该用户名已存在！";
        res.end(errors);
      } else {
        ...
        if (errors) {
          ...
          // 密码加密
          req.body.password = db.DbOpt.encrypt(req.body.password, config.encryptKey);
          req.body.group    = new db.AdminGroup({_id: req.body.group});
          db.DbOpt.addOne(db.AdminUser, req, res);
        }
      }
    })
  } else {
...
router.get('/manage/backupDataManage/delItem', function (req, res) {
  ...
  if (shortid.isValid(targetId)) {
    if (adminFunc.checkAdminPower(req, settings.backUpData[0] + '_del')) {
      db.DataOptionLog.remove({_id: targetId}, function (err, result) {
        if (err) {
          res.end(err);
        } else {
          if (forderPath) {
...
router.get('/manage/ContentList/topContent', function (req, res) {
  ...
  if (shortid.isValid(contentId)) {
    if (adminFunc.checkAdminPower(req, settings.contentList[0] + '_top')) {
      db.Content.update({_id: contentId}, {'isTop': isTop}, function (err, result) {
        if (err) throw  err;
        res.end("success");
      })
    } else {
...
function addOneCategory(req, res) {
  var errors;
  var newObj = new db.ContentCategory(req.body);
...
function addOneContentTags(req, res) {
  ...
  var query = db.ContentTags.find().or([{'name': name}, {alias: alias}]);
  // 标签或别名不允许重复
  query.exec(function (err, tags) {
    if (tags.length > 0) {
      ...
      db.DbOpt.addOne(db.ContentTags, req, res);
    }
  });
}
...
function addOneContentTemps(req, res) {
  var name = req.body.name;
  db.ContentTemplate.find({'name': name}, function (err, temp) {
    if (err) {
      res.end(err);
    } else {
      if (temp && temp.length > 0) {
        ...
        db.DbOpt.addOne(db.ContentTemplate, req, res);
      }
    }
  });
}
          ...
          if (err) {
            ...
            download_file_httpget(file_url, function () {
              ...
              extract.on('finish', function () {
                console.log("解压完成!!");
                //解压完成处理入库操作
                var tempItem       = new db.TemplateItems();
                ...
                tempItem.save(function (err) {
                  if (err) {
                    ...
                    var newTemp   = new db.ContentTemplate(tempObj);
                    newTemp.using = false;
...
router.post('/manage/updateCMSTemplate', function (req, res) {
  ...
  db.AdminUser.findOne({'_id': adminId}).populate('group').exec(function (err, doc) {
    if (err) {
      res.end(err);
    } else {
      var power  = false;
      ...
      if (power) {
        ...
        system.uploadTemp(req, res, function (fname) {
          ...
          fs.mkdir(DOWNLOAD_DIR, function (err1) {
            if (err1) {
              console.log(err1);
            }
            else {
...
router.get('/manage/enableTemp', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.contentTemps[0] + '_modify')) {
    ...
    if (!distPath) {
      ...
      db.ContentTemplate.setTempState('', false, function (err) {
        if (err) {
          res.end(err);
        } else {
          db.ContentTemplate.setTempState(tempId, true, function (err1, doc) {
            if (err1) {
              res.end(err1);
            } else {
              //复制静态文件到公共目录
              ...
              system.copyForder(fromPath, targetPath);
              db.ContentTemplate.getDefaultTemp(res, function (temp) {
                //重置类别模板
                db.ContentCategory.update({}, {$set: {contentTemp: ''}}, {multi: true}, function (err2) {
                  if (err2) {
                    res.end(err2);
                  } else {
                    res.end('success');
...
router.post('/manage/templateItem/addNew', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.CONTENTTEMPITEMS[0] + '_add')) {
    var tempItem = new db.TemplateItems(req.body);
    tempItem.save(function (err) {
      if (err) {
        res.end(err);
      } else {
...
function removeTemplateItem(req, res) {
  ...
  var targetId = params.query.uid;
  if (shortid.isValid(targetId)) {
    db.TemplateItems.remove({_id: params.query.uid}, function (err, result) {
      if (err) {
        res.end(err);
      } else {
        db.ContentTemplate.getDefaultTemp(res, function (doc) {
...
function removeTemplate(req, res) {
  ...
  if (shortid.isValid(targetId)) {
    adminFunc.delRefObjById(res, db.ContentTemplate, db.TemplateItems, targetId, function (doc) {
      //删除模板文件夹
...
router.get('/manage/contentTemps/tempFolderList', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.contentTemps[0] + '_view')) {
    db.ContentTemplate.find({}).sort({using: -1}).populate('items').exec(function (err, docs) {
      if (err) {
        res.end(err);
      } else {
...
function replyMessage(req, res) {
  ...
  if (replyEmail && !utils.validator.isEmail(replyEmail)) {
    errors = settings.system_illegal_param;
  }
  ...
  if (errors) {
    ...
    req.body.adminAuthor = new db.AdminUser({_id: adminAuthorId, userName: req.session.adminUserInfo.userName});
    req.body.replyAuthor = new db.User({_id: replyId, email: replyEmail});
    var newMsg           = new db.Message(req.body);
    newMsg.save(function () {
      // 更新评论数
      db.Content.updateCommentNum(contentId, 'add', function () {
        // 给用户发送提醒邮件
...
router.post('/manage/adsItems/addItem', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_add')) {
    db.Ads.findOne({'_id': adsId}, function (err, doc) {
      if (doc) {
        db.newAdsItem = new db.AdsItems(req.body);
        db.newAdsItem.save(function (err) {
          if (err) {
            res.end(err);
          } else {
            doc.items.push(db.newAdsItem);
            doc.save(function (err1) {
              if (err1) {
                res.end(err1);
              }
...
router.post('/manage/adsItems/modifyItem', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_modify')) {
    db.Ads.findOne({'_id': adsId}, function (err, doc) {
      if (doc) {
        db.DbOpt.updateOneByID(db.AdsItems, req, res, '');
      } else {
        res.end(settings.system_illegal_param);
      }
    })
...
router.get('/manage/adsItems/delItem', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_del')) {
    adminFunc.delSonRefObjById(res, db.Ads, db.AdsItems, adsId, params.query.uid, function () {
      res.end("success");
    });
  } else {
    res.end(settings.system_noPower);
...
router.get('/manage/adsItems/findItem', function (req, res) {
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_view')) {
    db.DbOpt.findOne(db.AdsItems, req, res, '');
  } else {
    res.end(settings.system_noPower);
  }
});
...
function addOneAds(req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_add')) {
    var adsItemObj = new db.AdsItems({
    ...
    adsItemObj.save(function (err) {
      if (err) {
        ...
        var adsObj = new db.Ads(req.body);
...
router.post('/manage/sysTemManage_ads/modifyTxtAds', function (req, res) {
  ...
  var link   = req.body.link;
  if (adminFunc.checkAdminPower(req, settings.adsList[0] + '_modify')) {
    db.Ads.findOneAndUpdate({'_id': adsId}, {$set: {'name': name, 'type': type, 'state': state}}, function (err, doc) {
      if (err) {
        res.end(err);
      }
      if (doc) {
        db.AdsItems.update({'_id': doc.items[0]}, {$set: {'title': title, 'link': link}}, function (err1) {
          if (err1) {
            res.end(err1)
          } else {
            res.end('success');
...
function removeAds(req, res) {
  ...
  adminFunc.delRefObjById(res, db.Ads, db.AdsItems, targetId, function () {
    res.end('success');
  });
...
function addOneNotice(req, res) {
  req.body.type        = '1';
  req.body.adminSender = new db.AdminUser({_id: req.session.adminUserInfo._id});
  var notify           = new db.Notify(req.body);
  notify.save(function (err) {
    if (err) {
      res.end(err);
    } else {
      db.User.find({}, '_id', function (err, users) {
        if (err) {
          res.end(err);
        } else {
          if (users.length > 0) {
            for (var i = 0; i < users.length; i++) {
              var userNotify    = new db.UserNotify();
...
router.get('/userNotify/setHasRead', function (req, res) {
  ...
  if (adminFunc.checkAdminPower(req, settings.sysTemBackStageNotice[0] + '_modify')) {
    if (currentId) {
      db.UserNotify.setHasRead(currentId, function (err) {
        if (err) {
          res.end(err);
        } else {
          adminFunc.getAdminNotices(req, res, function (noticeObj) {
