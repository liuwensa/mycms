...
// 用户登录提交请求
  router.post('/doLogin', function (req, res, next) {
    ...
    var errors;
    var newPsd   = DbOpt.encrypt(password, config.encryptKey);
    if (!utils.validator.isEmail(email)) {
      errors = '邮箱格式不正确';
    }
    if (!utils.validator.isPsd(password) || !utils.validator.isLength(password, 6, 12)) {
      errors = "密码6-12个字符";
    }
    if (errors) {
      res.end(errors);
...
//    获取当前发送邮件的时间
    ...
    if (!utils.validator.isEmail(targetEmail)) {
...
//点击找回密码链接跳转页面
  router.get('/reset_pass', function (req, res) {
    ...
    if (keyArr && utils.validator.isEmail(keyArr[1])) {
    ...
    if (keyArr && utils.validator.isEmail(keyArr[1])) {
      User.findOne({'email': keyArr[1]}, function (err, user) {
        if (err) {
          ...
          if (user.password == keyArr[0] && keyArr[2] == settings.session_secret
            && utils.validator.isPsd(req.body.password) && utils.validator.isLength(req.body.password, 6, 12)) {

            user.password = DbOpt.encrypt(req.body.password, config.encryptkey);
            user.save(function (err) {
              if (err) {
                res.end(err)
              } else {
  ...
  router.get('/userReplies/:defaultUrl', function (req, res) {
    if (isLogined(req)) {
      ...
      if (replyUrl == 'p') {
        ...
        if (replyPage && utils.validator.isNumeric(replyPage)) {
...
//                针对有密码的记录，需要解密后再返回
          if (result && result.password) {
            var decipher    = crypto.createDecipher("bf", config.encryptkey);
            var oldPsd      = "";
...
//修改用户信息
  router.post('/userInfo/modify', function (req, res, next) {
    ...
    if (name && (!utils.validator.isGBKName(name) || !utils.validator.isLength(name, 1, 5))) {
      errors = "姓名格式不正确";
    }
    ...
    if (city && (!utils.validator.isGBKName(city) || !utils.validator.isLength(city, 0, 12))) {
      errors = "请填写正确的城市名称";
    }

    if (company && (!utils.validator.isGBKName(company) || !utils.validator.isLength(company, 0, 12))) {
      errors = "请填写正确的学校中文名称";
    }

    if (qq && !utils.validator.isQQ(qq)) {
      errors = "请填写正确的QQ号码";
    }

    if (phoneNum && !utils.validator.isMobilePhone(phoneNum, 'zh-CN')) {
      errors = "请填写正确的手机号码";
    }
...
//密码修改
  router.post('/resetMyPsd', function (req, res, next) {
    ...
    if (!utils.validator.isPsd(oldPassword) || !utils.validator.isLength(oldPassword, 6, 12)) {
      errors = "6-12位，只能包含字母、数字和下划线";
    }
    if (!utils.validator.isPsd(userPsd) || !utils.validator.isLength(userPsd, 6, 12)) {
      errors = "6-12位，只能包含字母、数字和下划线";
    }
    ...
    if (errors) {
      ...
      //    密码加密
      var oldPsd = DbOpt.encrypt(oldPassword, config.encryptkey);
      var newPsd = DbOpt.encrypt(userPsd, config.encryptkey);
